...
void exploreTrail(int i, int j, int step, vector<pair<int,int>> &peaks_reached, vector<vector<int>> map){
    if(map[i][j] == 9 && step == 9){
        peaks_reached.push_back(make_pair(i,j));
        return;
    }
    if(map[i][j] != step){
        return;    
...
int getTrails(vector<vector<int>> map, int i, int j){
    ...
    vector<pair<int,int>> peaks_reached;
