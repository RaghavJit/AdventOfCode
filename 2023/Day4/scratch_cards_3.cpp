...
int getScore(int matches){
  ...
  for(int i=0; i<matches; i++){
    score = (matches==0) ? 0 : ((score==0) ? 1 : score*2);
  }
  return score;
}
...
int getRecScore(string line, int CardNum){
  int matches = getMatches(line);
  int RecScore = getScore(matches);
  for(int i=CardNum+1; i<=CardNum+matches; i++){
    RecScore = RecScore + getRecScore(getCard(i), i);
  }
  return RecScore;
}
...
int main(){
  ...
  while(getline(inputFile, line)){
    cout<<line<<'\n';
    GrandTotal = GrandTotal + getRecScore(line, CardNum);
    CardNum++;
  }
  inputFile.close();
